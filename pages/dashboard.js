...
export async function getServerSideProps(context) {
  ...
  console.log("project_running", project_running)
...
function Dashboard({ project_details, project_hold, project_completed, project_running, User_name, project_runn, project_h, project_comp, language, languageDepartment, priority, status }) {
  console.log('project_hold', project_hold)
  const { register, watch, handleSubmit, formState: { errors }, setValue } = useForm();
  const router = useRouter();


  // let allProjects=async()=>{
  //   let p_data=await axios.get('http://localhost:3000/api/project')
  //   const res=await p_data.data
  // }
                                                  ...
                                                  <GridItem>
                                                    <div className="delete_popup">
                                                      <CardBody>
                                                        ...
                                                        <h4 className={'cardTitleWhite'}>Are you sure you want to archive {project.project_title}?</h4>
                                                      </CardBody>
